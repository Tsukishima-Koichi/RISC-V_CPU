...
module myCPU (
    ...
    always_comb begin
        case (mem_funct3) 
            3'b000, 3'b100: mem_rdata_align = perip_rdata >> (8 * mem_agu_res[1:0]); // 修改为 AGU
            3'b001, 3'b101: mem_rdata_align = perip_rdata >> (16 * mem_agu_res[1]);  // 修改为 AGU
            default:        mem_rdata_align = perip_rdata;
        endcase
    end
    
    Mask #(DATAWIDTH) mask_inst (
        .mask(mem_funct3), .dout(mem_rdata_align), .mdata(mem_rdata_ext)
    );
